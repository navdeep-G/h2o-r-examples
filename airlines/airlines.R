...
scatter_plot(data = airlines.hex, x = "Distance", y = "TravelTime", max_points = 10000)


## Impute missing travel times by the "Origin" and "Dest" columns and re-plot. 
h2o.impute(data = airlines.hex, method = "mean", column = "Distance", by = c("Origin", "Dest"))
scatter_plot(data = airlines.hex, x = "Distance", y = "TravelTime", max_points = 10000)
...
myY <- "IsDepDelayed"
myX <- c("Origin", "Dest", "Year", "UniqueCarrier", "DayOfWeek", "Month", "Distance")
...
data.gbm <- h2o.gbm(x = myX, y = myY, training_frame = data.train, validation_frame = data.test, balance_classes = T, 
                    ntrees = 30, distribution = "bernoulli", learn_rate = 0.1)
